...
const Form: React.FC = () => {
    ...
    log(form);
    return (
        <div className='container mt-20 flex justify-between items-center mx-auto px-8 md:px-14 lg:px-24 w-full'>
                ...
                <div className="w-full grid lg:grid-cols-2 gap-8 lg:gap-32 mt-24">
                    <div className="space-y-12">
                        <div>
                            ...
                            <input type="text" className="w-full border border-input-border bg-slate-700 px-4 py-4" name="name" onChange={handleChange} value={form.name} />
                        </div>
                        <div>
                            <label htmlFor="" className='text-white block mb-6 text-xl font-bold'>Email</label>
                            <input type="email" className="text-white block mb-6 text-xl font-bold w-full border bg-slate-700 border-input-border px-4 py-1" name="email" onChange={handleChange} value={form.email}/>
                        </div>
                        <div>
                            <label htmlFor="" className='text-white block mb-6 text-xl font-bold'>Message</label>
                            <textarea name="message" id="" cols={30} rows={10} className="w-full border border-input-border bg-slate-700 px-4 py-1" onChange={handleChange} value={form.message}></textarea>
                        </div>
                        <button className='px-6 py-2 bg-blue-500 border text-white font-bold' onClick={sendEmail}>Send It!</button>
                    </div>
